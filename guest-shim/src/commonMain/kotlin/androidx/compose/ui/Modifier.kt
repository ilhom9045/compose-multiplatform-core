...
// Deviation from the verbatim copy: upstream's `Modifier.Node` and the imports that serve it are
// removed. Node is the integration point with the host's layout/draw node system — DelegatableNode,
// NodeCoordinator, NodeKind — none of which exists in the guest, and app code never names it: it is
// reached only by authors of custom ModifierNodeElement subclasses. The chain itself (Modifier,
// Element, CombinedModifier, the companion) is pure data and is copied unchanged.
