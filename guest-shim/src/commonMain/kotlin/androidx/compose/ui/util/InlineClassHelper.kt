...
// platform source set. The guest has a single leaf js source set with nothing beneath it to hold
// actuals, so the nonJvm/web body is inlined here unchanged.
...
inline fun doubleFromBits(bits: Long): Double = Double.fromBits(bits)
 ...
 */
inline fun Float.fastRoundToInt(): Int = if (isNaN()) 0 else roundToInt()
 ...
 * - +Infinity or any value greater than Integer.MAX_VALUE becomes Integer.MAX_VALUE.toFloat()
 */
inline fun Double.fastRoundToInt(): Int = if (isNaN()) 0 else roundToInt()
